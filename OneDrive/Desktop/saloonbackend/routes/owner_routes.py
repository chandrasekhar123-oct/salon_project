...
def add_self_worker():
    ...
    new_worker = Worker(
        ...
        experience=5, # arbitrary default for owner
        skill="All Services",
        is_approved=True,
        password=current_user.password, # owner can use the same pass or change it later via worker portal
        image_url=current_user.profile_image or "https://i.pravatar.cc/300"
    )
    db.session.add(new_worker)
    db.session.commit()
    flash('You have been added as a worker successfully. You can now login to the Worker App.', 'success')
    return redirect(url_for('owner.manage_workers'))
